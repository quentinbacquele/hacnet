...
from hacnet.minimal.modules.encoder import CausalConvEncoder
...
from hacnet.minimal.modules.mainnet import TransformerMainNet
from hacnet.minimal.modules.dechunker import EMADechunker
from hacnet.minimal.modules.decoder import CausalDecoder
...
class HACNetMinimal(nn.Module):
    def __init__(self, config: HACNetMinimalConfig):
        ...
        self.encoder = CausalConvEncoder(config.encoder, hidden_dim=hidden_dim)
        self.router = MultiLagRouter(config.router)
        self.chunker = BoundaryChunker(config.chunker)
        self.mainnet = TransformerMainNet(config.mainnet)
        self.dechunker = EMADechunker()
        self.decoder = CausalDecoder(config.decoder)
